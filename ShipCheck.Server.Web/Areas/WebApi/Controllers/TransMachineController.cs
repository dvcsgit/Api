...
{
    /// <summary>
    /// 轉單 將原本 電的類型的單據 轉成機的單據
    /// </summary>
    public class TransMachineController : ApiController
    {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    using (QIS_NEWEntities db = new QIS_NEWEntities())
                    {
                        ...
                        if (record != null)
                        {
                            if(string.IsNullOrEmpty(record.REPAIR_PERSON))
                            {
                                var beforeType = record.EXCEPTION_TYPE;
                                var afterType = beforeType == "M" ? "E" : "M";

                                record.EXCEPTION_TYPE = afterType;
                                record.TRANS_PERSON = model.Account;
                                record.TRANS_TIME = DateTime.Now.ToString("yyyyMMddHHmmss");

                                db.SaveChanges();

                                _logger.Info("此單 原本:{0} ==> 修改後 :{1}", beforeType, afterType);
                            }
                            else
                            {
                                throw new Exception(string.Format("此資料已在{0}被:{1}確認過,無法轉單", record.REPAIR_TIME, record.REPAIR_PERSON));
                            }
