...
using System.Linq;
using System.Web;
using FTC.SI.Web.Utils.Attributes;
                ...
                foreach (var subItem in columnName)
                {
                    ...
                    if (proType.FullName.Contains("System.DateTime"))
                    {
                        if (nullableFlag)
                        {
                            ...
                            {
                                propertyValue = tempDate.Value.ToString(Resource.StrDateTimeFormat);
                            }
                        }
                        else
                        {
                            var tempDate = (DateTime)sourceData.GetProperty(subItem).GetValue(item, null);
                            propertyValue = tempDate.ToString(Resource.StrDateTimeFormat);
                        }
                    }
